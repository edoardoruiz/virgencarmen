...
class TurnosMayorController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        if ($user->hasRole('admin')) {
            // Obtén todos los turnos si el usuario es un administrador
            $turnos = TurnosMayor::all();
        } else {
            // Obtén los turnos asignados al usuario actual
            $turnos = $user->turnos;
        }
    ...
    public function store(Request $request)
    {
        ...
        $turno = TurnosMayor::find($validated['turno_id']);
    
        if ($turno && $turno->disponibilidad >= $quantity) {
            // Obtener el último número de turno vendido
            $ultimo_turno_vendido = TurnosDetalle::where('turno_id', $validated['turno_id'])
                ->orderBy('id', 'desc')
                ->first();
            ...
            return redirect()->route('turnosdetalle.index')->with('success', 'Turnos comprados exitosamente.');
        }
    
        return redirect()->route('turnos.index')->with('error', 'No hay suficiente disponibilidad de turnos.');
    }
